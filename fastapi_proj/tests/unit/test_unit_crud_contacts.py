...
from app.source.schemas.contacts import (
    ContactBaseModel,
    ContactSearchUpdateModel,
    ResponseContactModel,
)
from app.source.db.models import User, Contact
from app.source.crud.crud_contacts import (
    get_contact,
    get_contacts,
    create_contact,
    delete_contact,
...
class TestCrudContacts(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_contact(self):
        ...
        self.async_session.execute.return_value = MagicMock(
            scalar_one=MagicMock(return_value=contact)
        )
    ...
    async def test_create_contact(self):
        ...
        self.async_session.execute.return_value = MagicMock(
            scalar_one=MagicMock(return_value=new_contact)
        )
    ...
    async def test_get_contacts(self):
        ...
        result = await get_contacts(
            search, self.user, self.async_session, limit, offset
        )
    ...
    async def test_delete_contact(self):
        ...
        test_contact = Contact(id=contact_id)
        self.async_session.execute.return_value = MagicMock(
            scalar_one=MagicMock(return_value=test_contact)
        )
    ...
    async def test_update_contact(self):
        ...
        await update_contact(
            test_contact.id, update_model, self.user, self.async_session
        )
        ...
        updated_contact = await get_contact(
            test_contact.id, self.user, self.async_session
        )
        ...
        for field in update_data:
            self.assertEqual(
                getattr(updated_contact, field), getattr(update_model, field)
            )
