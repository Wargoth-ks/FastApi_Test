...
from fastapi_proj.source.db.models import User


@pytest.mark.asyncio
async def test_signup(client_test: AsyncClient, user: dict, monkeypatch, mock_cloudinary_upload):
    
    mock_send_email = AsyncMock()
    monkeypatch.setattr("fastapi_proj.source.routes.routes_auth.email_service", mock_send_email)
...
@pytest.mark.asyncio    
async def test_repeat_signup(client_test: AsyncClient, user: dict, mock_cloudinary_upload):
...
async def test_refresh_token(client_test: AsyncClient, user: dict, create_test_session):
    ...
    response = await client_test.get("/api/auth/refresh_token", headers={"Authorization": f"Bearer {refresh_token}"})
    assert response.status_code == 200, response.text
    received_data = response.json()
    assert "access_token" in received_data
    assert "refresh_token" in received_data
...
async def test_logout(client_test: AsyncClient, login_tokens):
    ...
    response = await client_test.get("/api/auth/logout", headers={"Authorization": f"Bearer {access_token}"})
    assert response.status_code == 200, response.text
    received_data = response.json()
